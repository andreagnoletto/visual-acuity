...
const LANDOLT_OPTOTYPES: LandoltOptotype[] = [
  { type: 'C', rotation: 0, label: 'C gap top' },
  { type: 'C', rotation: 45, label: 'C gap top-right' },
  { type: 'C', rotation: 90, label: 'C gap right' },
  { type: 'C', rotation: 135, label: 'C gap bottom-right' },
  { type: 'C', rotation: 180, label: 'C gap bottom' },
  { type: 'C', rotation: 225, label: 'C gap bottom-left' },
  { type: 'C', rotation: 270, label: 'C gap left' },
  { type: 'C', rotation: 315, label: 'C gap top-left' }
];
...
const getLandoltGap = (rotation: number): string => {
  const gapSize = 50;
  const arcSize = 201;
  return `${arcSize} ${gapSize}`;
};

const getLandoltOffset = (rotation: number): number => {
  const circumference = 251;
  const offset = -(rotation / 360) * circumference;
  return offset;
};
